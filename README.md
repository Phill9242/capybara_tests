...
Por fim, é importante notar que a depender do tempo para os testes serem executados, alguns comportamentos podem variar de acordo com o ambiente em que os testes estão sendo executados. Isto porque, ao lidar com processos assíncronos ao executar scripts na página, o tempo de resposta para renderizar determinada página pode ultrapassar o limite de 2 segundos colocados por padrão. Esses erros podem ser comuns ao tentar verificar elementos após clicar em um botão que renderiza a página, carrega elementos, etc. Talvez seja necessário aumentar o tempo de resposta dentro da sua [configuração](#configuração_capybara), modificando o tempo limite de resposta, como no exemplo abaixo, onde o tempo máximo de espera foi modificado pra 5 segundos:
